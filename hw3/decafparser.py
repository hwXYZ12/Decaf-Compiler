...
def p_param(p):
	'param : type var'
	p[0] = Parameter(p[1], p[2], p.lineno, p.lineno)
	pass
...
def p_block(p):
	...
	if( isinstance(stmtList, list) and len(stmtList)>0):
		...
		p[0] = BlockStatement(stmtList, p.lineno, p.lineno)
	pass
def p_block_error(p):
...
def p_stmt_list_empty(p):
	'stmt_list : '
	p[0] = [SkipStatement(p.lineno, p.lineno)]
	pass
def p_stmt_list(p):
...
def p_stmt_return(p):
	...
	p[0] = ReturnStatement(p[2], p.lineno, p.lineno)
	pass
...
def p_stmt_break(p):
	...
	p[0] = BreakStatement(p.lineno, p.lineno)
	pass
def p_stmt_continue(p):
	'stmt : CONTINUE SEMICOLON'
	p[0] = ContinueStatement(p.lineno, p.lineno)
	pass
def p_stmt_block(p):
	'stmt : block'
	p[0] = p[1]
...
def p_stmt_var_decl(p):
	...
	# the print out!
	p[0] = VariableDeclarationStatement(p[1], p.lineno, p.lineno)
	pass
def p_stmt_error(p):
...
def p_literal_int_const(p):
	'literal : INT_CONST'
	p[0] = ConstantIntegerExpression(int(p[1]), p.lineno, p.lineno)
	pass
def p_literal_float_const(p):
	'literal : FLOAT_CONST'
	p[0] = ConstantFloatExpression(float(p[1]), p.lineno, p.lineno)
	pass
...
def p_field_access_dot(p):
	...
	p[0] = FieldAccessExpression(p[1], p[3], p.lineno, p.lineno)
	pass
...
def p_field_access_id(p):
	...
	p[0] = VariableExpression(p[1], p.lineno, p.lineno)
	pass
...
def p_assign_post_inc(p):
	'assign : lhs INC'
	p[0] = AutoExpression(p[1], "increment", "post-increment", p.lineno, p.lineno)
	pass
def p_assign_pre_inc(p):
	'assign : INC lhs'
	p[0] = AutoExpression(p[2], "increment", "pre-increment", p.lineno, p.lineno)
	pass
def p_assign_post_dec(p):
	'assign : lhs DEC'
	p[0] = AutoExpression(p[1], "decrement", "post-decrement", p.lineno, p.lineno)
	pass
def p_assign_pre_dec(p):
	'assign : DEC lhs'
	p[0] = AutoExpression(p[2], "decrement", "pre-decrement", p.lineno, p.lineno)
	pass
