...
class MethodRecord:
	...
	def getAMI(self, containingClass, declaration, breakLabel, continueLabel):
		...
		s += self.block.getAMI(containingClass, self)
		
		return s
		
	def getAMILabel(self):
		return "M_"+self.name+"_"+self.id
...
class IfElseStatement:
	...
	def getAMI(self, containingClass, declaration, breakLabel, continueLabel):
		...
		labelx = declaration.getAMILabel()+"_"+declaration.branchCounter
		declaration.branchCounter += 1
		labely = declaration.getAMILabel()+"_"+declaration.branchCounter
		
		# we're going to assume that the result of the evaluation of the
		# expression is going to go in the a0 register
		condition = self.cond.getAMI(self, containingClass, declaration, breakLabel, continueLabel)
		
		# get a new temporary register
		declaration.tempRegCounter += 1
		tx = declaration.tempRegCounter
		
		# first we evaluate the condition expression and move it's value
		# into our new temporary register
		s = condition + "\n"
		s += "move "+tx+", "+"a0\n"
		
		# now we add the branching structure described above
		s += "bz "+tx+", "+labelx+"\n"
		s += self.thenStmt.getAMI(self, containingClass, declaration, breakLabel, continueLabel) + "\n"
		s += "jmp "+labely+"\n"
		s += labelx+":\n"
		s += self.elseStmt.getAMI(self, containingClass, declaration, breakLabel, continueLabel) + "\n"
		s += labely+":"
		return s
...
class IfStatement:
	...
	def getAMI(self, containingClass, declaration, breakLabel, continueLabel):
		...
		# we're going to assume that the result of the evaluation of the
		# expression is going to go in the a0 register
		condition = self.expr.getAMI(self, containingClass, declaration, breakLabel, continueLabel)
		...
		# now we add the branching structure described above
		s += "bz "+tx+", "+labelx+"\n"
		s += self.stmt.getAMI(self, containingClass, declaration, breakLabel, continueLabel) + "\n"
		s += labelx+":"
		return s
...
class WhileStatement:
	...
	def getAMI(self, containingClass, declaration, breakLabel, continueLabel):
		...
		# generate labels
		declaration.branchCounter += 1
		startLoop = declaration.getAMILabel()+"_"+declaration.branchCounter
		
		declaration.branchCounter += 1
		endLoop = declaration.getAMILabel()+"_"+declaration.branchCounter
		
		# we're going to assume that the result of the evaluation of the
		# conditional expression is going to be in the a0 register
		condition = self.expr.getAMI(self, containingClass, declaration, breakLabel, continueLabel)
		...
		# we assume the condition expression moves it's evaluated
		# result into register a0
		s += condition + "\n"
...
class ForStatement:
	...
	def getAMI(self, containingClass, declaration, breakLabel, continueLabel):
		...
		# we're going to assume that the result of the evaluation of the
		# conditional expression is going to be in the a0 register
		condition = self.loopCond.getAMI(self, containingClass, declaration, breakLabel, continueLabel)
		
		# start with the init expression
		s = self.init.getAMI(self, containingClass, declaration, breakLabel, continueLabel) + "\n"
		...
		# we evaluate the condition expression and assume the result is in
		# register a0
		s += condition + "\n"
		
		# now we add the rest of the branching structure described above
		s += "bz a0+", "+endLoop+"\n"
		s += self.body.getAMI(self, containingClass, declaration, breakLabel, continueLabel) + "\n"
		s += self.update.getAMI(self, containingClass, declaration, breakLabel, continueLabel) + "\n"
		s += "jmp "+startLoop+"\n"
		s += endLoop+":"
		return s
...
class ReturnStatement:
	...
	def getAMI(self, containingClass, declaration, breakLabel, continueLabel):
		# return statements merely evaluate their return expressions
		# and append a 'ret' instruction
		# it is assumed that the evaluated expression will move the evaluation
		# to register a0 so it will not be included here
		s = self.ret.getAMI(self, containingClass, declaration, breakLabel, continueLabel)+"\n"
		s += "ret"
		return s
...
class ExpressionStatement:
	...
	def getAMI(self, containingClass, declaration, breakLabel, continueLabel):
		return self.expr.getAMI(self, containingClass, declaration, breakLabel, continueLabel)
...
class VariableDeclarationStatement:
	...
	def getAMI(self, containingClass, declaration, breakLabel, continueLabel):
		# variable declaration statements are only useful during AST
		# construction and do not serve a purpose in AMI conversion
		return ""
...
class ConstantIntegerExpression:
	...
	def getAMI(self, containingClass, declaration, breakLabel, continueLabel):
		# move the integer into the a0 register for evaluation
		return "move_immed_i a0, "+ info
...
class ConstantFloatExpression:
	...
	def getAMI(self, containingClass, declaration, breakLabel, continueLabel):
		# move the float into the a0 register for evaluation
		return "move_immed_f a0, "+ info
...
class ConstantNullExpression:
	...
	def getAMI(self, containingClass, declaration, breakLabel, continueLabel):
		# I'm not too sure about how this ought to be handled
		# essentially this expression ought to evaluate to a null expression
		# but I'm not certain how null pointers should be represented
		# For now I'm going to use 0 to represent a null pointer
		return "move_immed_i a0, 0"
...
class VariableExpression:
	...
	# TODO ....
	def getAMI(self, containingClass, declaration, breakLabel, continueLabel):
		# I'm not sure about this one I think that we're going to have
		# the variable that this expression points to be something in a
		# register and this will return that register as a string
		return "0"
